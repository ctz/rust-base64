...
pub fn encode_config_buf<T: ?Sized + AsRef<[u8]>>(input: &T, config: Config, buf: &mut String) {
    ...
    // reserve to make sure loops below are allocation-free
    ...
    if input_bytes.len() >= 8 {
        while input_index <= last_fast_index {
            ...
            buf.push(charset[((input_chunk >> 58) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 52) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 46) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 40) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 34) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 28) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 22) & 0x3F) as usize]);
            buf.push(charset[((input_chunk >> 16) & 0x3F) as usize]);
    ...
    while leftover_index < start_of_rem {
        buf.push(charset[(input_bytes[leftover_index] >> 2) as usize]);
        buf.push(charset[((input_bytes[leftover_index] << 4 |
                           input_bytes[leftover_index + 1] >> 4) & 0x3f) as usize]);
        buf.push(charset[((input_bytes[leftover_index + 1] << 2 |
                           input_bytes[leftover_index + 2] >> 6) & 0x3f) as usize]);
        buf.push(charset[(input_bytes[leftover_index + 2] & 0x3f) as usize]);

        leftover_index += 3;
    }

    if rem == 2 {
        buf.push(charset[(input_bytes[start_of_rem] >> 2) as usize]);
        buf.push(charset[((input_bytes[start_of_rem] << 4 |
                           input_bytes[start_of_rem + 1] >> 4) & 0x3f) as usize]);
        buf.push(charset[(input_bytes[start_of_rem + 1] << 2 & 0x3f) as usize]);
    } else if rem == 1 {
        buf.push(charset[(input_bytes[start_of_rem] >> 2) as usize]);
        buf.push(charset[(input_bytes[start_of_rem] << 4 & 0x3f) as usize]);
    }
